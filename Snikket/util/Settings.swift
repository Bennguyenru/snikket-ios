...
public enum Settings: String {
    ...
    public static let sharedDefaults = UserDefaults(suiteName: "group.com.hxg.italks.share");
                ...
                if k.starts(with: "upload-") {
                    ...
                    if let timestamp = (v as? [String: Any])?["timestamp"] as? Date {
                        if timestamp < removeOlder {
                            ...
                            let localUploadDirUrl = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.com.hxg.italks.shared")!.appendingPathComponent("upload", isDirectory: true).appendingPathComponent(hash, isDirectory: false);
